...
EMPIRICAL_OUT = REPO_ROOT / "statistical-modeling/outputs/final-model-summaries"
PREDICTIVE_OUT = REPO_ROOT / "statistical-modeling/outputs/heldout-evaluation/final-predictions"
STIMULUS_DIR = REPO_ROOT / "statistical-modeling/outputs/stimulus-model"
FEATURE_DIR = REPO_ROOT / "statistical-modeling/outputs/feature-model"
N33_HELDOUT_DIR = REPO_ROOT / "statistical-modeling/outputs/heldout-evaluation/mcmc-evaluation"
PHASE6H1_DIR = REPO_ROOT / "llm-experiments/outputs/final/evaluation"
REAL_DATA_DIR = REPO_ROOT / "statistical-modeling/data/real"
...
def consolidate_empirical_results(repo_root: Path, output_dir: Path) -> dict[str, Any]:
    stimulus_manifest = read_json(repo_root / "statistical-modeling/outputs/stimulus-model/real_stimulus_model_manifest.json")
    feature_manifest = read_json(repo_root / "statistical-modeling/outputs/feature-model/real_feature_model_manifest.json")
    stimulus_fixed = pd.read_csv(repo_root / "statistical-modeling/outputs/stimulus-model/fixed_effect_posterior_summary.csv")
    feature_fixed = pd.read_csv(repo_root / "statistical-modeling/outputs/feature-model/primary_fixed_effect_posterior_summary.csv")
    stimulus_var = pd.read_csv(repo_root / "statistical-modeling/outputs/stimulus-model/final_model_variance_components.csv")
    stimulus_icc = pd.read_csv(repo_root / "statistical-modeling/outputs/stimulus-model/final_model_icc_summary.csv")
    feature_var = pd.read_csv(repo_root / "statistical-modeling/outputs/feature-model/variance_components.csv")
    feature_icc = pd.read_csv(repo_root / "statistical-modeling/outputs/feature-model/icc_posterior_summary.csv")
    stimulus_diag = pd.read_csv(repo_root / "statistical-modeling/outputs/stimulus-model/convergence_diagnostics.csv")
    feature_diag = pd.read_csv(repo_root / "statistical-modeling/outputs/feature-model/convergence_diagnostics.csv")
    ...
    inventory = {
        ...
        "stimulus_model_location": "statistical-modeling/outputs/stimulus-model",
        "feature_model_location": "statistical-modeling/outputs/feature-model",
...
def freeze_n33_predictive_baseline(repo_root: Path, output_dir: Path) -> dict[str, Any]:
    candidate = pd.read_csv(repo_root / "statistical-modeling/outputs/heldout-evaluation/mcmc-evaluation/candidate_predictions.csv")
    split = pd.read_csv(repo_root / "statistical-modeling/outputs/heldout-evaluation/mcmc-evaluation/heldout_split_manifest.csv")
    leakage = pd.read_csv(repo_root / "statistical-modeling/outputs/heldout-evaluation/mcmc-evaluation/leakage_audit.csv")
    diagnostics = pd.read_csv(repo_root / "statistical-modeling/outputs/heldout-evaluation/mcmc-evaluation/fold_diagnostics.csv")
    eval_manifest = read_json(repo_root / "statistical-modeling/outputs/heldout-evaluation/mcmc-evaluation/evaluation_manifest.json")
    h1_ground_truth = pd.read_csv(repo_root / "llm-experiments/outputs/final/evaluation/heldout_ground_truth.csv")
    h1_metric = read_json(repo_root / "llm-experiments/outputs/final/evaluation/metric_protocol.json")
...
def build_split_manifest(split: pd.DataFrame, h1: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        ...
        "source": "statistical-modeling/outputs/heldout-evaluation/mcmc-evaluation/heldout_split_manifest.csv",
        "phase6h1_source": "llm-experiments/outputs/final/evaluation/heldout_ground_truth.csv",
...
def build_model_config(eval_manifest: dict[str, Any], h1_metric: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "metric_protocol_reference": "llm-experiments/outputs/final/evaluation/metric_protocol.json",
        "metric_protocol_hash": sha256_file(PHASE6H1_DIR / "metric_protocol.json"),
...
def build_prediction_freeze_manifest(
    ...
    source_files = [
        ...
        "statistical-modeling/outputs/heldout-evaluation/mcmc-evaluation/heldout_split_manifest.csv",
        ...
        "llm-experiments/outputs/final/evaluation/metric_protocol.json",
...
def build_completion_gates(empirical: dict[str, Any], predictive: dict[str, Any]) -> dict[str, bool]:
    ...
    gates = {
        ...
        "N33_INCOMPLETE_MARKERS_RESOLVED": all(item["classification"] != "GENUINELY_INCOMPLETE" for item in empirical["incomplete_marker_audit"]["markers"]),
        **pred_gates,
    }
    return gates
...
def incomplete_marker_audit(repo_root: Path, stimulus_manifest: dict[str, Any], feature_manifest: dict[str, Any], convergence: dict[str, Any]) -> dict[str, Any]:
    ...
    for marker_path, manifest, converged in [
        (repo_root / "statistical-modeling/outputs/stimulus-model/INCOMPLETE_PENDING_QMUL_RERUN.txt", stimulus_manifest, convergence["stimulus_converged"]),
        (repo_root / "statistical-modeling/outputs/feature-model/INCOMPLETE_PENDING_QMUL_RERUN.txt", feature_manifest, convergence["feature_converged"]),
    ]:
    ...
    return {
        ...
        "bookkeeping_action": "Stale incomplete markers were removed during Supporting Material cleanup; absence is the expected clean state.",
        "markers": markers,
    }
