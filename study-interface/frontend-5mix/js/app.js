...
window.StudyApp.app = (function () {
  ...
  function initialisePage() {
    ...
    prepareStoredStateForCurrentConfig().then(function () {
      continueInitialisePage(currentPage);
    }).catch(function (error) {
      console.error("Stored state preparation failed.", error);
      continueInitialisePage(currentPage);
    });
  }

  function continueInitialisePage(currentPage) {
    var fallback;

    if (window.StudyApp.navigation && !window.StudyApp.navigation.isRouteAllowed(currentPage)) {
      fallback = window.StudyApp.navigation.getRouteFallback(currentPage);
      if (fallback && fallback !== window.location.pathname.split("/").pop()) {
  ...
  function prepareStoredStateForCurrentConfig() {
    if (!window.StudyApp.storage || typeof window.StudyApp.storage.validateStoredStateAgainstConfig !== "function") {
      return Promise.resolve();
    }
    return fetch("../config/stimuli.json", { cache: "no-store" }).then(function (response) {
      if (!response.ok) {
        throw new Error("Could not load stimuli config.");
      }
      return response.json();
    }).then(function (config) {
      window.StudyApp.storage.validateStoredStateAgainstConfig(config);
    }).catch(function () {
      return fetch("config/stimuli.json", { cache: "no-store" }).then(function (response) {
        if (!response.ok) {
          throw new Error("Could not load root stimuli config.");
        }
        return response.json();
      }).then(function (config) {
        window.StudyApp.storage.validateStoredStateAgainstConfig(config);
  ...
  function handleFinalSubmit(currentPage) {
    ...
    window.StudyApp.netlifySubmission.buildAndSubmit().then(function (payload) {
      ...
      setError(summary, true);
      if (summary) {
        summary.textContent = error && error.localPreviewSubmissionUnavailable === true ?
          "This local preview cannot save to Netlify Forms. Your payload validated, but real submission must be tested on a Netlify deploy or deploy preview. Your answers have not been cleared." :
          "Your responses could not be saved. Please check your connection and try Final Submit again. Your answers have not been cleared.";
        summary.focus();
      }
    });
