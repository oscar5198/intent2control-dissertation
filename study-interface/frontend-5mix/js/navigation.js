...
window.StudyApp.navigation = (function () {
  ...
  function isRouteAllowed(pageId) {
    ...
    if (hasFinalSubmission() && pageId !== "completion" && pageId !== "landing") {
      return false;
    }
    ...
    if (pageId === "review") {
      return Boolean(window.StudyApp.storage &&
        hasCompletedExperimentalTrials() &&
        window.StudyApp.storage.getItem("postTask.completed") === true &&
        window.StudyApp.storage.getItem("demographics.completed") === true);
    }
  ...
  function getRouteFallback(pageId) {
    if (hasFinalSubmission() && pageId !== "landing" && pageId !== "completion") {
    ...
    if (pageId === "completion") {
      return hasFinalSubmission() ? "completion.html" : "review.html";
    }
    return "index.html";
  }
