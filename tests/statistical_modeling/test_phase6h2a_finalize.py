...
def build_tmp(tmp_path: Path) -> dict:
    phase6h2a.EMPIRICAL_OUT = tmp_path / "final-model-summaries"
    phase6h2a.PREDICTIVE_OUT = tmp_path / "final-predictions"
...
def test_authoritative_n33_empirical_models_and_formulas(tmp_path: Path) -> None:
    ...
    assert empirical["stimulus_model_location"] == "statistical-modeling/outputs/stimulus-model"
    assert empirical["feature_model_location"] == "statistical-modeling/outputs/feature-model"
...
def test_incomplete_markers_are_absent_or_not_genuinely_incomplete(tmp_path: Path) -> None:
    result = build_tmp(tmp_path)
    audit = result["empirical"]["incomplete_marker_audit"]

    assert audit["bookkeeping_action"] == "Stale incomplete markers were removed during Supporting Material cleanup; absence is the expected clean state."
    assert len(audit["markers"]) == 2
    assert all(not marker["exists"] for marker in audit["markers"])
    assert all(marker["classification"] == "ABSENT" for marker in audit["markers"])


def test_fixed_effect_variance_and_icc_outputs_exist(tmp_path: Path) -> None:
    result = build_tmp(tmp_path)
    empirical_dir = tmp_path / "final-model-summaries"
...
def test_prediction_files_have_990_candidates_198_trials_and_no_scores(tmp_path: Path) -> None:
    result = build_tmp(tmp_path)
    predictive_dir = tmp_path / "final-predictions"
...
def test_winner_and_ranking_are_derived_from_predicted_ratings(tmp_path: Path) -> None:
    build_tmp(tmp_path)
    candidate = pd.read_csv(tmp_path / "final-predictions" / "final_n33_candidate_predictions.csv")
    trial = pd.read_csv(tmp_path / "final-predictions" / "final_n33_trial_predictions.csv")
...
def test_freeze_manifest_hashes_and_metric_protocol_reference(tmp_path: Path) -> None:
    result = build_tmp(tmp_path)
    freeze = load_json(tmp_path / "final-predictions" / "final_n33_prediction_freeze_manifest.json")
    config = load_json(tmp_path / "final-predictions" / "final_n33_model_config.json")
    ...
    assert config["metric_protocol_reference"] == "llm-experiments/outputs/final/evaluation/metric_protocol.json"
    assert config["metric_protocol_hash"] == phase6h2a.sha256_file(REPO_ROOT / "llm-experiments/outputs/final/evaluation/metric_protocol.json")
    assert result["gates"]["MIXED_EFFECTS_LLM_FAIR_COMPARISON_READY"] is True


def test_final_n33_baseline_is_primary(tmp_path: Path) -> None:
    result = build_tmp(tmp_path)

    assert "old_n30_baseline" not in result["manifest"]
    assert result["predictive"]["split_manifest"]["participant_count"] == 33
    assert result["predictive"]["split_manifest"]["heldout_trial_count"] == 198
