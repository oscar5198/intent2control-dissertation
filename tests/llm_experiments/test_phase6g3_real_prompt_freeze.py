...
OUT = REPO_ROOT / "llm-experiments" / "outputs" / "final" / "rendered-prompts"
PROMPT_DATA = REPO_ROOT / "llm-experiments" / "outputs" / "final" / "prompt-data" / "final_prompt_dataset.jsonl"
PHASE6G1_MANIFEST = REPO_ROOT / "llm-experiments" / "outputs" / "final" / "prompt-data" / "prompt_data_manifest.json"
FREEZE = OUT / "prompt_freeze_manifest.json"
RENDERED = OUT / "rendered_final_prompts.jsonl"
HASH_MANIFEST = OUT / "prompt_hash_manifest.json"
REQUESTS = OUT / "request_manifest.json"
READINESS_6G2D = REPO_ROOT / "llm-experiments" / "outputs" / "final" / "inference-config" / "readiness.json"
...
def test_condition_equivalence_and_leakage_audits_pass() -> None:
    freeze = load_json(FREEZE)
    leakage = freeze["leakage_status"]

    assert freeze["matched_pair_count"] == 198
    assert freeze["valid_pair_count"] == 198
    assert leakage["condition_equivalence_failures"] == 0
    assert leakage["comment_boundary_failures"] == 0
...
def test_size_and_context_compatibility_pass() -> None:
    freeze = load_json(FREEZE)
    context = freeze["context_compatibility_status"]
    context_rows = {row["model_key"]: row for row in context["models"]}

    assert freeze["condition_counts"] == {"non_history": 198, "personalised_history": 198}
...
def test_centaur_serialization_and_choice_marker_decision() -> None:
    centaur = load_json(FREEZE)["context_compatibility_status"]["centaur_serialization"]
...
def test_request_matrix_and_shards_complete_without_predictions() -> None:
    ...
    assert load_json(OUT / "gpt_request_shard_manifest.json")["request_count"] == 396
    assert load_json(OUT / "claude_request_shard_manifest.json")["request_count"] == 396
    assert load_json(OUT / "llama_request_shard_manifest.json")["request_count"] == 396
    assert load_json(OUT / "qmul_all_request_shard_manifest.json")["request_count"] == 1188
    assert load_json(OUT / "centaur_request_shard_manifest.json")["request_count"] == 396


def test_ground_truth_isolation_and_no_secret_artifacts() -> None:
    for path in [FREEZE, HASH_MANIFEST, REQUESTS]:
