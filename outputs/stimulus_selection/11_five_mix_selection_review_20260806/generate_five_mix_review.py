...
OUT = ROOT / "outputs/stimulus_selection/11_five_mix_selection_review_20260806"
FIG = OUT / "figures"
PATHS = OUT / "song_review_paths"
FIG.mkdir(parents=True, exist_ok=True)
PATHS.mkdir(parents=True, exist_ok=True)

SOURCES = {
    "current_pool": "outputs/stimulus_selection/04_mix_selection_v2/tables/acoustic_candidate_pool.csv",
    ...
    "six_proposals": "outputs/stimulus_selection/09_six_mix_proposals/tables/six_mix_proposals.csv",
    ...
    "six_audio": "outputs/stimulus_selection/09_six_mix_proposals/tables/six_mix_audio_manifest.csv",
    ...
    "rtb_review": "outputs/stimulus_selection/09_six_mix_proposals/reports/final_alignment_and_red_to_blue_review.md",
    "backup_pool": "outputs/stimulus_selection/08_backup_song_expansion/04_acoustic_candidate_pools/tables/acoustic_candidate_pool_backup.csv",
    ...
    "backup_pairwise_alignment": "outputs/stimulus_selection/08_backup_song_expansion/08_supervisor_review_package/tables/pairwise_alignment_verification_backup.csv",
}
...
for song in final_songs:
    ...
    if song in CURRENT:
        ...
        base = ROOT / "outputs/stimulus_selection/08_backup_song_expansion/08_supervisor_review_package"
        audio = {r.original_mix_name: str(base / str(r.review_audio_path)) for _, r in backup_review[backup_review.song == song].iterrows()}

    pd.DataFrame(
        [{"song": song, "original_mix_name": n, "audio_path": audio.get(n, ""), "do_not_duplicate_audio": "path_reference_only"} for n in names]
    ).to_csv(PATHS / f"{song.replace(' ', '_').replace(chr(39), '').lower()}_audio_paths.csv", index=False)
...
pd.DataFrame(eval_rows).to_csv(OUT / "candidate_song_evaluation.csv", index=False)
pd.DataFrame(combo_rows).drop(columns=["pairs"], errors="ignore").to_csv(OUT / "five_mix_combination_scores.csv", index=False)
rank.to_csv(OUT / "red_to_blue_replacement_ranking.csv", index=False)
...
pd.DataFrame(role_rows).to_csv(OUT / "brecht_ratings_and_roles.csv", index=False)
pd.DataFrame(checklist_rows).to_csv(OUT / "oscar_listening_checklist.csv", index=False)
summary = pd.DataFrame(summary_rows)
summary.to_csv(OUT / "recommended_set_summary.csv", index=False)
...
plot_rank = rank.sort_values("replacement_rank_score")
plt.barh(plot_rank.song, plot_rank.replacement_rank_score, color="#6b8ba4")
plt.xlabel("Composite evidence score")
plt.title("Red To Blue vs backup candidates")
for i, r in enumerate(plot_rank.itertuples()):
    plt.text(r.replacement_rank_score, i, f"  min d={r.min_pairwise_distance:.2f}, range={r.rating_range:.2f}", va="center", fontsize=8)
plt.tight_layout()
plt.savefig(FIG / "red_to_blue_replacement_ranking.png", dpi=180)
plt.close()

fig, axes = plt.subplots(1, 2, figsize=(12, 4))
summary.plot(kind="bar", x="song", y=["minimum_pairwise_distance", "median_pairwise_distance", "maximum_pairwise_distance"], ax=axes[0])
axes[0].set_title("Selected five-mix acoustic distances")
axes[0].set_ylabel("combined Euclidean distance")
axes[0].tick_params(axis="x", rotation=25)
summary.plot(kind="bar", x="song", y="rating_range", ax=axes[1], color="#8a7a5c", legend=False)
axes[1].set_title("Selected five-mix Brecht rating range")
axes[1].set_ylabel("mean preference range")
axes[1].tick_params(axis="x", rotation=25)
fig.tight_layout()
fig.savefig(FIG / "recommended_set_summary.png", dpi=180)
plt.close(fig)

for song in final_songs:
    u = selected[song]
    feat = derived(u["features"]).copy()
    f = feat[feat.original_mix_name.isin(list(u["pool"].original_mix_name))].copy()
    f = f.merge(u["ratings"][["original_mix_name", "mean_previous_preference"]], on="original_mix_name", how="left")
    xcol = "combined_bark_pc_01" if "combined_bark_pc_01" in f.columns else "bark_pc_01"
    ycol = "combined_scalar_stereo_width" if "combined_scalar_stereo_width" in f.columns else "scaled_stereo_width"
    fig, ax = plt.subplots(figsize=(7, 5))
    pts = ax.scatter(f[xcol], f[ycol], s=50, c=f.mean_previous_preference, cmap="viridis", edgecolor="black", linewidth=0.5)
    for _, r in f.iterrows():
        ax.text(r[xcol], r[ycol], " " + r.original_mix_name, fontsize=8, fontweight="bold" if r.original_mix_name in u["names"] else "normal")
    ax.set_title(f"{song}: candidate space (selected labels bold)")
    ax.set_xlabel(xcol)
    ax.set_ylabel(ycol)
    fig.colorbar(pts, ax=ax, label="Brecht mean preference")
    fig.tight_layout()
    fig.savefig(FIG / f"{song.replace(' ', '_').replace(chr(39), '').lower()}_candidate_space.png", dpi=180)
    plt.close(fig)

visual_refs = []
for song in final_songs + ["Red To Blue"] + BACKUPS:
    for rel in [
        f"outputs/stimulus_selection/09_six_mix_proposals/diagnostics/{song}/pairwise_distance_heatmap.png",
        f"outputs/stimulus_selection/09_six_mix_proposals/diagnostics/{song}/six_mix_acoustic_overview.png",
        f"outputs/stimulus_selection/09_six_mix_proposals/alignment_review/{song}_six_mix_waveforms.png",
    ]:
        if (ROOT / rel).exists():
            visual_refs.append({"song": song, "path": rel, "type": "existing_reference"})
pd.DataFrame(visual_refs).to_csv(OUT / "existing_visual_reference_paths.csv", index=False)
...
lines += [
    ...
    "- `brecht_ratings_and_roles.csv`",
    "- `candidate_song_evaluation.csv`",
    "- `red_to_blue_replacement_ranking.csv`",
    "- `oscar_listening_checklist.csv`",
    "- `recommended_set_summary.csv`",
    "- `authoritative_sources_manifest.csv`",
    "- `figures/` generated review plots",
    "- `song_review_paths/` audio path references",
]
